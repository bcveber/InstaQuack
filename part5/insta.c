...
void* Subscriber(void *args){
    ...
    //initialize all html writers with NULL
    for (int i = 0; i < MAXTOPICS; i++)
        out[i] = NULL;
    ...
    while(fscanf(fp, "%d %d", &topic, &seconds) > 0){
        ...
        //if we have a new writer, then initialize it
        if (out[topic] == NULL){
            //generate file name
            sprintf(name, "subscriber_%d_topic_%d.html", info->id, topic);

            //open file and error check
            if ((out[topic] = fopen(name, "w+")) == NULL){
                //if writing failed, then ignore this command
                printf("failed to open '%s'\n", name);
                continue;
            }

            //write html header
            fprintf(out[topic], HTML_HEADER_FMT, topic, info->id, topicQueue[topic].name);
        }
        ...
        if (t != 5){
            ...
            fprintf(out[topic], HTML_BODY_FMT, entry.photoUrl, entry.photoCaption);
        }
    ...
    //write the html footer to all open writers and close them
    for (int i = 0; i < MAXTOPICS; i++){
        if (out[i]){
            //write footer
            fputs(HTML_FOOTER_FMT, out[i]);
